...
from ussy_assay import __version__
from ussy_assay.grade import grade_project, grade_module, compute_trends, grade_label
from ussy_assay.compose import compose_function, compose_module, compose_bar
from ussy_assay.alloy import detect_alloys, find_pure_functions, analyze_project_alloys
from ussy_assay.crucible import build_crucible, crucible_rank_emoji
from ussy_assay.slag import (
    detect_slag_in_project,
    detect_slag_in_source,
    grade_improvement_estimate,
)
from ussy_assay.storage import save_analysis, load_latest_run
...
from ussy_assay.scanner import _resolve_paths
...
def _cmd_slag(args: argparse.Namespace) -> None:
    ...
    if project.total_lines > 0:
        ...
        print(
            f"Potential grade improvement: {project.grade:.0f}% \u2192 {improved:.0f}% (slag removal only)"
        )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    grade_parser = subparsers.add_parser(
        "grade", help="Grade report — measure precious logic percentage"
    )
