...
from ussy_churn.communities import TerritorySummary
from ussy_churn.layout import build_layout


def test_build_layout_creates_grid() -> None:
    graph = nx.Graph()
    graph.add_node("core", commit_count=3, commit_hashes={"a"}, files={"a.py"}, authors={"alice"})
    graph.add_node("ui", commit_count=2, commit_hashes={"b"}, files={"b.py"}, authors={"bob"})
