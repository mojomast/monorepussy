...
def _find_repo_root(path: str) -> Optional[Path]:
    ...
    from seral.git_utils import find_repo_root as _find
    return _find(path)
    ...
    for m in results:
        if m.stage:
            ...
            if old_stage and old_stage != m.stage.value:
                from seral.models import StageTransition
