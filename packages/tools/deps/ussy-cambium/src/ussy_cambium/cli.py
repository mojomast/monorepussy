...
from ussy_cambium.bond import bond_trajectory, compute_bond_strength, format_bond_report
...
from ussy_cambium.dwarfing import DependencyNode, format_dwarfing_report
from ussy_cambium.extractor import extract_interface, extract_interface_from_file
...
from ussy_cambium.scanner import format_scan_report, scan_project
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    # bond-traj
    bond_parser = subparsers.add_parser("bond-traj", help="Integration bond strength trajectory")
...
def cmd_dwarfing(args: argparse.Namespace) -> int:
    ...
    from ussy_cambium.scanner import _parse_dependencies, _build_dependency_tree
    ...
    if getattr(args, "json_output", False):
        from ussy_cambium.dwarfing import analyze_dependency_chain, compute_chain_capability
...
def main(argv: list[str] | None = None) -> int:
    ...
    if getattr(args, "version", False):
        from ussy_cambium import __version__

        print(f"ussy-cambium {__version__}")
        return 0
